...
def title(msg):
    ...
    chromedriver_dir = r"C:\Python39\chromedriver.exe"
...
@bot.event
async def on_ready():
    print('다음으로 로그인합니다:11 ')
    print(bot.user.name)
    print('connection was succesful')
    await bot.change_presence(status=discord.Status.online, activity=discord.Game("제5인격"))

@bot.event
async def on_command_error(ctx,error):
    await ctx.send('알수없는 명령어입니다.')
    pass    
...
async def 나가(ctx):
    try:
        ...
        await ctx.send("이미 채널에 속하지 않았습니다.")
...
async def 노래(ctx, *, url):
    ...
    try:
        ...
        try:
            await vc.move_to(ctx.message.author.voice.channal)
        except:
                await ctx.send("채널에 유저가 접속하지않았습니다.")
...
async def 노래검색(ctx, *, msg):
    ...
    await ctx.send(embed = discord.Embed(title="노래 검색중..", description = "느려도 이해해주세요..!", color = 0x00ff00))
    if not vc.is_playing():
...
async def 스킵(ctx):
    if vc.is_playing():
        vc.stop()
        await ctx.send(embed = discord.Embed(title= "스킵", description = musicnow[0]  + "을(를) 했습니다.", color = 0x00ff00))
    else:
        await ctx.send("지금 노래가 재생되지 않네요.")

@bot.command()
async def 백만볼트(ctx):
    await ctx.send("쿠르릉 쾅!!")

@bot.command()
async def 도움말(ctx):
    await ctx.send(embed = discord.Embed(title='도움말',description="""
\n!도움말 -> 모든 명령어를 볼 수 있습니다.
\n!나가 -> 네오를 자신이 속한 채널에서 내보냅니다.
\n!노래 [노래링크] -> 유튜브URL를 입력하면 노래를 틀어줍니다.
(목록재생에서는 사용 못함)
\n!노래검색 [노래이름] -> 노래를 검색해 틀어줍니다.
\n!노래끄기 -> 재생중인 노래를 끕니다.
\n!스킵 ->노래를 스킵합니다
...
!즐겨찾기추가 [노래이름] -> 노래를 검색해 즐겨찾기에 추가합니다.
...
bot.run("ODk2NjAxODMzODM2MDAzMzQ4.YWJfhw.B7GjGpMBJbH8bv9FcZ776RlwKz0")
